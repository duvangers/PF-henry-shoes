...
export function updateOrden(payload, ordenId) {
  return async function (dispatch) {
    try {
      const response = await Axios.put(`/ordens/${ordenId}`, payload);
      Sweal.fire("Actualización", `${response.data.msg}`, "success");
      return {
        type: actionTypes.UPDATE_ORDEN,
      };
    } catch (error) {
      console.log(error);
      Sweal.fire("Ups hubo un error", `${error.response.data.msg}`, "error");
